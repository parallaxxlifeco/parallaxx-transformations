...
#
#     REBUILD_SUPERSEDED=1 python3 build-reconnected-woman-bundle.py
#
...
# then PtNav v3, then PtFooter v3.
CSS = "\n\n".join(style_block(i) for i in opens[1:])

# The markup now opens with the nav, not the page root: this page carries its
# own chrome so it is ONE custom element in Wix, the same as every other page.
h0 = next(i for i, l in enumerate(lines) if l.startswith('<header id="pt-nav">'))
...
for fn in ("function initPtNav(", "function initPtFooter("):
    if fn not in body:
        sys.exit(f"FAIL: {fn}...) missing from the bundle body. This page carries "
                 f"its own chrome; without the initialisers the nav renders and "
...
if '<footer id="pt-foot">' not in HTML:
    sys.exit("FAIL: the footer markup is not in the extracted HTML.")
if "#pt-bar{" not in CSS:
    sys.exit("FAIL: the nav stylesheet did not make it into CSS.")
