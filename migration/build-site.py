...
def main() -> int:
    ...
    # Every root-level bundle, listed by name rather than matched by a glob.
    bundle_paths = sorted("/" + f.name for f in REPO.glob("*.js"))
    bundle_rules = "".join(
    ...
    (DIST / "_headers").write_text(
        ...
        "#\n"
        "# LISTED ONE BY ONE ON PURPOSE. This rule used to be /*.js, which reads\n"
        "# like it should match and does not -- the bundles kept coming back with\n"
        "# max-age=14400 while the /assets/* rule directly above was applied, so\n"
        "# the file was being read and the pattern was the thing at fault. A splat\n"
        "# in Pages matches a path prefix, not a file extension.\n"
        + bundle_rules,
        encoding="utf-8",
    )
    print("headers  %d bundles pinned to max-age=0" % len(bundle_paths))
