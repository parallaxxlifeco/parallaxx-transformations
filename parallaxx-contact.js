/* PARALLAXX TRANSFORMATIONS - Contact. Custom Element. Tag: parallaxx-contact.
...
(function(){
  if (customElements.get('parallaxx-contact')) return;

  var CSS = `/* ═══════════════════════════════════════════════════════════════════
   PARALLAXX TRANSFORMATIONS  ·  CONTACT
   -------------------------------------------------------------------
   The most linked page on the site. It is in the nav on every page, it
   is the coral pill in the top right of every page, it is in the
   footer utility row: 24 links in all.

   WHAT THIS PAGE IS NOT ANY MORE
   The old 'Parallaxx Contact.dc.html' carried a form that called
   preventDefault(), threw all four fields away and redirected to a
   booking page. It never sent anything, on Wix or anywhere. So there
   is no form here. A direct mailto cannot silently swallow an
   enquiry, and there is nothing to maintain.

   THE SECOND PASS, 25 AUG: A CARD, NOT A HERO
   The first version put the email address on the page at headline
   size with a coral arrow beside it, and a tall portrait next to it.
   It was loud, and loud is the wrong register for the page somebody
   opens when they have already decided to get in touch. Daniel's note
   was 'make the contact links minimalistic, how you'd imagine a
   business card'.

   So the whole reach block is now a card, and the card behaves the
   way a real one does:

     - IDENTITY LEFT, DETAILS RIGHT, hairline between them. That is
       the fold of the card, and on a phone it stacks.
     - NO PRIMARY CHANNEL. Four rows, one weight, one colour. The
       copy says 'reach out through your preferred channel', so
       ranking them would argue with the sentence they sit under.
       This is also what removed the coral arrow that made the first
       version shout.
     - LABEL AND VALUE, like the back of a card: a fixed label column
       in small caps, the value beside it. Nothing is a button.
     - THE ONLY MOVEMENT IS ON HOVER, and it is one line of colour
       plus a small mark appearing. No sliding, no lift, no shadow.

   COLOUR IS DELIBERATELY ALMOST ABSENT HERE. Gold for the labels,
   because labels are Daniel's voice, and cream for the values. Coral
   appears on this page in exactly two places -- the rule across the
   top of the card, and the underline that arrives on hover. On a page
   whose job is to be easy rather than persuasive, restraint IS the
   design.

   THREE SECTIONS, ALTERNATING. Navy card, cream what-people-write-
   about, navy two-doors. Never two creams adjacent.
   ═══════════════════════════════════════════════════════════════════ */

  :where(#pc-root) *,:where(#pc-root) *::before,:where(#pc-root) *::after{box-sizing:border-box;margin:0;padding:0}
  #pc-root ::selection{background:#FF501F;color:#fff}
  ...
  .pc-serif{font-family:'Poppins','Montserrat',sans-serif;font-weight:400;letter-spacing:-.018em;line-height:1.18;color:var(--cream)}
  .pc-wrap{max-width:1240px;margin:0 auto;padding-left:clamp(20px,4vw,52px);padding-right:clamp(20px,4vw,52px)}
  .pc-col{max-width:660px;margin-left:auto;margin-right:auto}
  .pc-label{font-size:.66rem;letter-spacing:.22em;text-transform:uppercase;font-weight:700;color:var(--label)}
  .pc-gold-label{color:var(--gold);opacity:.9}
  #pc-root strong{font-weight:600;color:var(--cream)}
  .on-cream strong{color:var(--ink)}
  .sr{position:absolute;width:1px;height:1px;overflow:hidden;clip:rect(0 0 0 0);white-space:nowrap}

  /* ── REVEAL ──────────────────────────────────────────────────────
     No GSAP and no Lenis on this page. It is a utility page whose whole
     job is to hand over an address, and it cannot afford to lose that
     address to a CDN round trip that never lands. Plain CSS keyframes
     on a delay, so nothing is ever waiting on a library. */
  .pc-up{animation:pcUp .7s cubic-bezier(.22,.61,.36,1) both}
  @keyframes pcUp{from{opacity:0;transform:translateY(18px)}to{opacity:1;transform:translateY(0)}}
  @media(prefers-reduced-motion:reduce){
    #pc-root *,#pc-root *::before,#pc-root *::after{animation:none!important;transition:none!important}
  }

  /* ── 01 · THE CARD ───────────────────────────────────────────── */
  #pc-reach{background:var(--navy);
    padding:calc(clamp(30px,4.4vw,60px) + var(--pc-nav)) 0 clamp(40px,5.5vw,68px)}
  #pc-reach .pc-col{max-width:900px}
  #pc-reach h1{font-size:clamp(1.5rem,3.4vw,2.1rem);font-weight:400;margin:0;letter-spacing:-.022em}
  #pc-reach .lede{margin:14px 0 0;font-size:1rem;color:var(--mist);max-width:54ch;line-height:1.62}

  /* THE RATIO IS 0.68 / 1.32, NOT HALF AND HALF. The identity side is a
     portrait and two short lines; the details side has to hold a
     36-character email address on ONE line, and at 50/50 it does not --
     it broke at the @ and the card immediately looked like a form again.
     Everything about this split is downstream of that one string.

     align-items:center rather than start, because the two halves are
     within about thirty pixels of each other in height and topping them
     out left a visible hole under the details. */
  /* THE PORTRAIT IS A COLUMN, NOT A THUMBNAIL. First pass had it at 112px
     inside the identity block, which is correct for a printed business card
     and wrong for this business: somebody on this page is deciding whether to
     approach a person, and a person the size of a favicon does not help them
     do that. It is 300px now and it holds its own half of the card.

     A fixed track rather than a fraction, because the details side has a
     36-character email address to fit on one line and a fractional split
     hands it whatever is left over. This way the portrait takes what it
     needs and the address always has the rest. */
  .pc-card{margin:clamp(24px,3.2vw,34px) 0 0;
    border:1px solid var(--line);border-radius:6px;
    background:linear-gradient(180deg,rgba(10,29,60,.7),rgba(10,29,60,.3));
    padding:clamp(20px,2.8vw,32px);
    display:grid;grid-template-columns:minmax(180px,290px) minmax(0,1fr);
    gap:clamp(22px,2.8vw,36px);align-items:center}

  .pc-id{display:flex;flex-direction:column;gap:16px}
  .pc-shot{width:100%;aspect-ratio:1;border-radius:4px;overflow:hidden;
    border:1px solid rgba(232,198,95,.28);background:var(--navy-2);flex:none}
  /* height:auto releases the height attribute so aspect-ratio can win --
     the same trap the first version of this page hit, where the portrait
     rendered at its full intrinsic height and pushed the section apart. */
  .pc-shot img{display:block;width:100%;height:auto;aspect-ratio:1;object-fit:cover;object-position:center 18%}
  .pc-name{font-family:'Poppins','Montserrat',sans-serif;font-weight:400;letter-spacing:-.015em;
    font-size:clamp(1.18rem,2.4vw,1.42rem);color:var(--cream);line-height:1.2}
  /* .14em, not .2em. At the wider tracking this ran to three lines in a
     230px column -- PERSONAL / LEADERSHIP / FACILITATOR -- which turned
     a job title into a stack. */
  .pc-role{margin-top:8px;font-size:.6rem;letter-spacing:.14em;text-transform:uppercase;
    font-weight:700;color:var(--gold);opacity:.9;line-height:1.7}
  /* This sits UNDER the card, not inside it. Inside, it made the identity
     column taller than the details column and put a hole in the layout;
     it is a footnote, so it reads as one. */
  .pc-where{margin:16px 0 0;font-size:.78rem;color:var(--label);line-height:1.6}

  /* ── THE DETAILS ─────────────────────────────────────────────────
     Label column fixed so the four values line up on one axis, which is
     the whole visual idea of the back of a card. Nothing here is styled
     as a button. */
  .pc-named{margin-bottom:16px}
  .pc-idtext{display:none}
  .pc-details{border-top:1px solid var(--line)}
  .pc-detail{display:grid;grid-template-columns:84px minmax(0,1fr) auto;gap:14px;
    align-items:baseline;padding:12px 2px;border-bottom:1px solid var(--line);
    text-decoration:none;transition:background .25s ease}
  .pc-detail:first-child{border-top:0}
  .pc-detail .k{font-size:.6rem;letter-spacing:.18em;text-transform:uppercase;font-weight:700;
    color:var(--gold);opacity:.75;transition:opacity .25s ease}
  .pc-detail .v{font-size:.95rem;color:var(--mist);line-height:1.45;overflow-wrap:break-word;
    transition:color .25s ease;border-bottom:1px solid transparent;padding-bottom:1px}
  .pc-detail .go{font-size:.82rem;color:transparent;transition:color .25s ease}
  .pc-detail:hover .k,.pc-detail:focus-visible .k{opacity:1}
  .pc-detail:hover .v,.pc-detail:focus-visible .v{color:var(--cream);border-bottom-color:var(--coral)}
  .pc-detail:hover .go,.pc-detail:focus-visible .go{color:var(--coral)}
  .pc-detail:focus-visible{outline:2px solid var(--gold);outline-offset:2px;border-radius:2px}

  /* ── 02 · WHAT PEOPLE WRITE ABOUT ──────────────────────────────
     Same 900px measure as the card above it, so the grid's outer edges
     land on the card's outer edges. At the default 660 the two blocks
     were centred on the same axis but a hundred and twenty pixels apart
     at the margins, which reads as a wobble rather than as a change of
     rhythm. */
  #pc-about{background:var(--cream);color:var(--ink-soft);padding:clamp(40px,5.5vw,68px) 0}
  #pc-about .pc-col{max-width:900px}
  #pc-about .pc-label{color:var(--bronze)}
  #pc-about h2{font-family:'Poppins','Montserrat',sans-serif;font-weight:400;letter-spacing:-.018em;
    line-height:1.2;color:var(--ink);font-size:clamp(1.3rem,2.7vw,1.8rem);margin:12px 0 0}
  /* ── THE FOUR, AS A GRID ─────────────────────────────────────────
     Was a stacked list, which made four equal options read as a ranked
     one -- the eye takes a vertical run of numbered rows as an order of
     preference, and only the first one really gets read. Two by two,
     they read as four doors instead, which is what they are.

     Hairlines rather than boxes, and the same construction as .sp-three
     on the Speaking page: a border-bottom on every cell and a
     border-left on the right-hand column only. Four cards on cream
     would fight the card in the section above it; this is quieter and
     it already exists elsewhere on the site.

     THE ORDER IS THE BUSINESS, NOT THE INBOX. Group work and 1:1 leads
     because the two doors are what the site actually sells; facilitation
     follows, and it is a credibility page rather than a sales page by
     Daniel's own call. */
  .pc-grid{margin:clamp(22px,3vw,32px) 0 0;display:grid;
    grid-template-columns:repeat(2,minmax(0,1fr));
    border-top:1px solid var(--line-cream)}
  .pc-grid > div{padding:clamp(18px,2.4vw,26px) clamp(18px,2.4vw,28px) clamp(18px,2.4vw,26px) 0;
    border-bottom:1px solid var(--line-cream)}
  .pc-grid > div:nth-child(2n){border-left:1px solid var(--line-cream);
    padding-left:clamp(20px,2.6vw,30px);padding-right:0}
  .pc-grid .n{font-family:'Poppins','Montserrat',sans-serif;font-size:.78rem;font-weight:600;
    letter-spacing:.08em;color:var(--bronze)}
  .pc-grid h3{margin:12px 0 8px;font-family:'Poppins','Montserrat',sans-serif;font-weight:500;
    letter-spacing:-.012em;font-size:1.08rem;color:var(--ink);line-height:1.28}
  .pc-grid p{color:var(--ink-soft);font-size:.94rem;line-height:1.62;max-width:38ch}
  #pc-about .after{margin:clamp(20px,2.8vw,28px) 0 0;color:var(--ink-soft);max-width:54ch;font-size:.99rem}
  #pc-about .after a{color:var(--coral-ink);text-decoration:underline;text-underline-offset:3px;font-weight:600}
  #pc-about .after a:hover{color:var(--coral)}

  /* ── PHONE ─────────────────────────────────────────────────────── */
  /* A card folds. Below 720 the two halves stack, the portrait and the
     name sit on one line so the identity block stays a single band
     rather than a tall column, and the label moves above its value --
     which is what gives the 36-character address the full width of the
     card and keeps it on one line down to 360. */
  /* BETWEEN 720 AND 980 THE PORTRAIT GIVES WAY FIRST. The card is still two
     columns here but the page has lost ~150px, and with the portrait holding
     its full 300px the details column drops to about 280 -- ten pixels under
     what the email address needs, so it broke to two lines through this band
     only. The portrait is the flexible one: it reads fine at 220, and the
     address has nowhere to go. */
  @media(max-width:980px){
    .pc-card{grid-template-columns:minmax(140px,24%) minmax(0,1fr)}
  }

  /* ON A PHONE THE PORTRAIT SHRINKS BACK. A full-width square here is 350px
     tall and pushes all four contact details below the fold, which is the
     exact opposite of what a page called Contact is for. It goes back to
     sitting inline with the name, where it reads as identity rather than as
     a picture. */
  @media(max-width:720px){
    .pc-card{grid-template-columns:1fr;gap:clamp(22px,4vw,28px)}
    .pc-id{flex-direction:row;align-items:center;gap:18px}
    .pc-shot{width:96px;flex:none}
    .pc-named{display:none}
    .pc-idtext{display:block}
    .pc-role{max-width:none}
    .pc-where{margin-top:10px;max-width:none}
    .pc-detail{grid-template-columns:minmax(0,1fr) auto;gap:4px 12px;padding:14px 2px}
    .pc-detail .k{grid-column:1/-1;margin-bottom:2px}
  }
  /* Below 420 the address stops fitting even at .9rem: the card loses its
     padding, the row loses the label column, and 36 characters still have to
     land on one line -- broken across two it reads as a typo in the one
     string on the site that has to look exact. It scales with the viewport
     from here down and clears 320 with room to spare. */
  @media(max-width:680px){
    .pc-grid{grid-template-columns:1fr}
    .pc-grid > div{padding:22px 0}
    .pc-grid > div:nth-child(2n){border-left:0;padding-left:0}
  }

  @media(max-width:420px){
    .pc-detail .v{font-size:min(.9rem,3.9vw)}
    .pc-shot{width:70px}
  }
  /* ON A PHONE THE WHOLE OPENING HAS TO FIT ONE SCREEN. Headline, the two
     lines of copy and all four channels, above the fold on a 390x844 handset
     with the nav still over the top of it. Everything below is a step down
     from the desktop rhythm rather than the same numbers squeezed: the
     heading loses four points, the card loses its outer padding, and the
     detail rows tighten from 12px to 9px, which is the difference between
     the fourth channel being on the screen and being under it. */
  @media(max-width:560px){
    #pc-reach{padding-top:calc(22px + var(--pc-nav));padding-bottom:34px}
    #pc-reach h1{font-size:1.5rem}
    #pc-reach .lede{font-size:.94rem;margin-top:11px;line-height:1.58}
    .pc-card{margin-top:20px;padding:18px 16px}
    .pc-detail{padding:9px 2px}
    .pc-named,.pc-idtext{margin-bottom:0}
    .pc-where{margin-top:12px;font-size:.74rem}
    #pc-about h2{font-size:1.3rem}
    .pc-grid p{max-width:none}
  }
...
<!-- ══════════════════════════════════════════════════════════════
     01 · THE CARD
     Rebuilt 25 Aug. The first version led with the email address at
     headline size beside a tall portrait, under the headline "Write to
     me." It was loud, and loud is the wrong register for the page
     somebody opens once they have already decided to make contact.

     This is a business card. Identity on the left, details on the
     right, a hairline where the fold would be. FOUR CHANNELS AT ONE
     WEIGHT, because the copy says "your preferred channel" and
     ranking them would argue with the sentence they sit under.

     The lede is Daniel's own words, used as written.
     ══════════════════════════════════════════════════════════════ -->
<section id="pc-reach">
  <div class="pc-wrap"><div class="pc-col">

    <h1 class="pc-serif pc-up">Contact</h1>
    <p class="lede pc-up" style="animation-delay:.1s">I am easy to contact, reach out through your preferred channel. Let me know what you&rsquo;ve got in mind, and let&rsquo;s have a conversation about it.</p>

    <div class="pc-card pc-up" style="animation-delay:.2s">

      <div class="pc-id">
        <figure class="pc-shot">
          <img src="https://static.wixstatic.com/media/e1784d_6f9d4aa43725445babf0c30004223730~mv2.jpg/v1/crop/x_0,y_48,w_1080,h_984/fill/w_280,h_255,al_c,q_80,usm_0.66_1.00_0.01,enc_avif,quality_auto/Daniel%20IG%20and%20Templates%20(1).jpg"
               alt="Daniel Lawson" width="1080" height="1080" loading="lazy" decoding="async">
        </figure>
        <div class="pc-idtext">
          <p class="pc-name">Daniel Lawson</p>
          <p class="pc-role">Personal Leadership &middot; Coaching &amp; Facilitation</p>
        </div>
      </div>
      ...
      <div class="pc-details">
        ...
        <a class="pc-detail" href="mailto:daniel@parallaxxtransformations.com">
          <span class="k">Email</span>
          <span class="v">daniel@<wbr>parallaxxtransformations.com</span>
          <span class="go" aria-hidden="true">&#8594;</span>
        </a>

        <a class="pc-detail" href="https://www.instagram.com/daniel.lawson__/" target="_blank" rel="noopener">
          <span class="k">Instagram</span>
          <span class="v">&#64;daniel.lawson__</span>
          <span class="go" aria-hidden="true">&#8599;</span>
        </a>

        <a class="pc-detail" href="https://www.linkedin.com/in/daniel-reconnect-you/" target="_blank" rel="noopener">
          <span class="k">LinkedIn</span>
          <span class="v">Daniel Lawson</span>
          <span class="go" aria-hidden="true">&#8599;</span>
        </a>

        <a class="pc-detail" href="https://www.youtube.com/@ReconnectYou1/featured" target="_blank" rel="noopener">
          <span class="k">YouTube</span>
          <span class="v">Reconnect You</span>
          <span class="go" aria-hidden="true">&#8599;</span>
        </a>
    ...
    <p class="pc-where pc-up" style="animation-delay:.3s">Clients across Europe, the US and Australia.</p>
...
<!-- ══════════════════════════════════════════════════════════════
     02 · WHAT COMES IN
     This block is doing the job the old form's dropdown was doing —
     telling somebody their thing belongs here — without a form to do
     it. Four items, because four is what actually arrives.
     ══════════════════════════════════════════════════════════════ -->
<section id="pc-about" class="on-cream">
  <div class="pc-wrap"><div class="pc-col">

    <p class="pc-label">Before you write</p>
    <h2>Most of what comes in is one of four things.</h2>

    <div class="pc-grid">

      <div>
        <p class="n">01</p>
        <h3>Group work and 1:1</h3>
        <p>Tell me what you&rsquo;re seeking, and let&rsquo;s find out what will be the best fit.</p>
      </div>

      <div>
        <p class="n">02</p>
        <h3>Event Host / Facilitation</h3>
        <p>From design consulting to experience curation, through the lens of creating deep transformational value for a wide range of audiences.</p>
      </div>

      <div>
        <p class="n">03</p>
        <h3>Podcasts / Panel Talks</h3>
        <p>Stories, frameworks and examples for personal leadership and intentional lifestyle design.</p>
      </div>

      <div>
        <p class="n">04</p>
        <h3>Something else?</h3>
        <p>A partnership, a press question, or something that fits nowhere above. Ask about it anyway.</p>
      </div>

    </div>

    <p class="after">If this is you, write to me &mdash; <a href="mailto:daniel@parallaxxtransformations.com">daniel@parallaxxtransformations.com</a></p>
...
<footer id="pt-foot">
  ...
  function addFonts(){
    ...
    var ff=document.createElement('style'); ff.id='pc-fonts'; ff.textContent=`/* LUMIOS MARKER is not on Google Fonts -- upload the .woff2 to Wix Media and
...
@font-face{font-family:'Lumios Marker';font-display:swap;
  ...
  function boot(root){
    if(!root || !root.getElementById('pc-root')) return;
...
/* ================================================================
  ...
  customElements.define('parallaxx-contact', ParallaxxContact);
})();
