...
(function(){
  ...
  var CSS = `/* ═══════════════════════════════════════════════════════════════════
   ...
   footer utility row, and it was the in-body CTA on the old site:
   24 links in all. Until now every one of them 301'd to the home page.
   ...
   enquiry, and there is nothing to maintain. If a LeadConnector embed
   is wanted later it is a ten-minute change, and it should sit
   BESIDE the address rather than replace it.

   THE ACCENT RULE, SAME AS EVERY OTHER PAGE
   GOLD  #E8C65F  everything Daniel says: labels, hairlines, the
                  handwriting, the small print under his name.
   CORAL #FF501F  the visitor's move. Here that is one thing only —
                  the email address. The socials are a quieter row
                  underneath it, because a DM is a worse channel for
                  the thing this page exists to catch.

   THREE SECTIONS, ALTERNATING. Navy reach-out, cream what-people-
   write-about, navy two-doors. Never two creams adjacent.
   ═══════════════════════════════════════════════════════════════════ */
  ...
  .pc-hand{font-family:'Lumios Marker','Permanent Marker',cursive;color:var(--gold);line-height:1.35}
  ...
  /* ── 01 · REACH ────────────────────────────────────────────────
     TWO COLUMNS ON DESKTOP, AND THE PORTRAIT IS NOT DECORATION.
     This page asks somebody to write to a person, and the version
     without a photograph asked them to write to an address. The rest
     of the page keeps the 660px measure the Priority Audit uses; only
     this section opens out, which reads as an editorial hero rather
     than an inconsistency.

     THE FRAME IS 'daniel-conversation.jpg' ON PURPOSE. The archive's
     other portraits are the home page hero -- Daniel behind glass,
     lit from inside, deliberately unreachable -- and one with his arms
     folded. Both are the wrong posture for the one page whose whole
     proposition is that he is reachable. This one is him mid
     conversation, looking straight out.

     The URL below is the Wix CDN original. migration/build-site.py
     rewrites it to /assets/img/daniel-conversation.jpg from
     asset-map.json, so it must stay in that exact form or the
     deployed page ships a broken portrait. */
  #pc-reach{background:var(--navy);
    padding:calc(clamp(56px,9vw,104px) + var(--pc-nav)) 0 clamp(56px,9vw,104px)}
  #pc-reach .grid{max-width:1080px;margin:0 auto;display:grid;
    grid-template-columns:minmax(0,1.06fr) minmax(0,.94fr);
    gap:clamp(34px,5vw,68px);align-items:center}
  #pc-portrait{position:relative;border-radius:20px;overflow:hidden;
    border:1px solid var(--line);background:var(--navy-2)}
  /* height:auto is load bearing. The <img> carries width/height attributes so
     the browser reserves the box before the file arrives, and that height wins
     over aspect-ratio unless it is released -- the portrait rendered 1500px
     tall and pushed the whole section off the screen. */
  #pc-portrait img{display:block;width:100%;height:auto;aspect-ratio:4/5;object-fit:cover;object-position:center 20%}
  #pc-portrait::after{content:'';position:absolute;inset:0;pointer-events:none;
    background:linear-gradient(180deg,rgba(4,18,42,0) 58%,rgba(4,18,42,.5) 100%)}
  #pc-reach h1{font-size:clamp(2.1rem,6.4vw,3.5rem);font-weight:400;margin:16px 0 0}
  #pc-reach h1 em{font-style:normal;color:var(--gold);font-family:'Lumios Marker','Permanent Marker',cursive;
    font-weight:400;letter-spacing:0}
  #pc-reach .lede{margin:22px 0 0;font-size:1.02rem;color:var(--mist);max-width:52ch;line-height:1.66}

  /* ── THE ROWS ────────────────────────────────────────────────────
     The old page had this shape and it was the one thing worth keeping:
     a stack of full-width rows that slide right on hover. Restyled onto
     the v4 palette. The email row is the only coral thing on the page. */
  .pc-rows{margin:clamp(34px,5vw,52px) 0 0;border-top:1px solid var(--line)}
  .pc-row{display:flex;align-items:center;justify-content:space-between;gap:18px;
    padding:22px 0;border-bottom:1px solid var(--line);text-decoration:none;
    transition:padding-left .3s ease,border-color .3s ease}
  .pc-row:hover,.pc-row:focus-visible{padding-left:12px;border-color:rgba(232,198,95,.42)}
  .pc-row:focus-visible{outline:2px solid var(--gold);outline-offset:3px}
  .pc-row .what{display:block;font-size:.64rem;letter-spacing:.2em;text-transform:uppercase;
    font-weight:700;color:var(--label);margin-bottom:8px}
  .pc-row .who{display:block;font-family:'Poppins','Montserrat',sans-serif;font-weight:400;
    letter-spacing:-.015em;font-size:clamp(1.12rem,3.1vw,1.62rem);color:var(--cream);
    line-height:1.2;overflow-wrap:break-word}
  .pc-row .go{flex:none;font-size:1.2rem;color:var(--slate);transition:color .3s,transform .3s}
  .pc-row:hover .go{color:var(--gold);transform:translateX(4px)}

  /* The address is the move, so it is the only row carrying coral. */
  .pc-row.is-primary .what{color:var(--gold);opacity:.9}
  .pc-row.is-primary .who{color:var(--cream);font-size:clamp(1.24rem,4vw,2rem)}
  .pc-row.is-primary .go{color:var(--coral)}
  .pc-row.is-primary:hover .go{color:var(--coral-lift)}
  .pc-row.is-primary:hover{border-color:rgba(255,80,31,.5)}

  .pc-social{display:flex;flex-wrap:wrap;gap:10px;margin:clamp(26px,4vw,36px) 0 0}
  .pc-social a{font-size:.76rem;font-weight:600;letter-spacing:.1em;text-transform:uppercase;
  ...
  .pc-social a:hover{border-color:var(--gold);color:var(--cream);background:rgba(232,198,95,.08)}
  .pc-social a:focus-visible{outline:2px solid var(--gold);outline-offset:3px}

  .pc-where{margin:clamp(26px,4vw,34px) 0 0;font-size:.78rem;letter-spacing:.06em;color:var(--label)}

  /* ── 02 · WHAT PEOPLE WRITE ABOUT ────────────────────────────── */
  #pc-about{background:var(--cream);color:var(--ink-soft);padding:clamp(56px,8vw,96px) 0}
  ...
  #pc-about h2{font-family:'Poppins','Montserrat',sans-serif;font-weight:400;letter-spacing:-.018em;
    line-height:1.18;color:var(--ink);font-size:clamp(1.5rem,3.4vw,2.15rem);margin:14px 0 0}
  .pc-list{margin:clamp(28px,4vw,40px) 0 0;border-top:1px solid var(--line-cream)}
  .pc-list li{list-style:none;padding:20px 0;border-bottom:1px solid var(--line-cream);
    display:grid;grid-template-columns:auto 1fr;gap:16px;align-items:baseline}
  .pc-list .n{font-family:'Poppins','Montserrat',sans-serif;font-size:.8rem;font-weight:600;
    letter-spacing:.08em;color:var(--bronze)}
  .pc-list p{color:var(--ink-soft);font-size:1rem;line-height:1.6;max-width:52ch}
  .pc-list b{font-weight:600;color:var(--ink);display:block;font-family:'Poppins','Montserrat',sans-serif;
    font-size:1.06rem;letter-spacing:-.01em;margin-bottom:4px}
  #pc-about .after{margin:clamp(26px,4vw,34px) 0 0;color:var(--ink-soft);max-width:54ch;font-size:.99rem}
  ...
  /* ── 03 · STILL LOOKING ──────────────────────────────────────── */
  #pc-doors{background:var(--navy-ink);padding:clamp(52px,7vw,84px) 0}
  #pc-doors h2{font-family:'Poppins','Montserrat',sans-serif;font-weight:400;letter-spacing:-.018em;
    color:var(--cream);font-size:clamp(1.32rem,3vw,1.8rem);margin:14px 0 0;line-height:1.24}
  #pc-doors p{margin:14px 0 0;color:var(--slate);font-size:.97rem;max-width:50ch}
  .pc-doors-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(240px,1fr));
    gap:14px;margin:clamp(28px,4vw,38px) 0 0}
  .pc-door{display:block;text-decoration:none;background:rgba(18,35,63,.5);
    border:1px solid var(--line);border-radius:16px;padding:26px 26px 24px;
    transition:border-color .3s,background .3s,transform .3s}
  .pc-door:hover{border-color:rgba(232,198,95,.45);background:rgba(18,35,63,.85);transform:translateY(-2px)}
  .pc-door:focus-visible{outline:2px solid var(--gold);outline-offset:3px}
  .pc-door .k{font-size:.64rem;letter-spacing:.2em;text-transform:uppercase;font-weight:700;color:var(--gold);opacity:.9}
  .pc-door .t{display:block;margin-top:12px;font-family:'Poppins','Montserrat',sans-serif;
    font-size:1.2rem;color:var(--cream);letter-spacing:-.01em;line-height:1.25}
  .pc-door .d{display:block;margin-top:10px;font-size:.9rem;color:var(--slate);line-height:1.55}

  /* ── PHONE ───────────────────────────────────────────────────── */
  /* THE ADDRESS IS 36 CHARACTERS AND IT IS THE PAGE.
     At any readable size it cannot sit on one line on a phone, so the
     question is only where it breaks. Left alone the browser breaks it
     mid-word -- 'parallaxxtransformation / s.com' -- which reads as a
     typo in the one string on the site that has to look exact. A <wbr>
     after the @ gives it a legal break point, overflow-wrap:break-word
     makes it prefer that point over an arbitrary one, and the size steps
     down so the domain half clears 320px on its own line. */
  @media(max-width:400px){
    .pc-row.is-primary .who{font-size:clamp(.95rem,4.4vw,1.24rem)}
  }

  /* AT DESKTOP THE ADDRESS SHARES ITS ROW WITH THE PORTRAIT, so the column
     it sits in is about 530px and the full 2rem size breaks it over two
     lines. It is the one string on the page that should read as a single
     object, so it steps down to fit. Below 900 the layout is one column,
     the row gets the full measure back, and the clamp above takes over. */
  @media(min-width:901px){
    .pc-row.is-primary .who{font-size:1.55rem}
  }

  /* Under 900 the two columns become one and the portrait falls to the
     bottom of the section, where a 4:5 crop would be a wall of image
     under the fine print. It becomes a band instead. */
  @media(max-width:900px){
    #pc-reach .grid{grid-template-columns:1fr;gap:clamp(30px,6vw,42px)}
    #pc-portrait img{aspect-ratio:3/2;object-position:center 22%}
  }

  @media(max-width:560px){
    #pc-reach{padding-top:calc(40px + var(--pc-nav));padding-bottom:52px}
    #pc-reach .lede{font-size:.97rem}
    .pc-row{padding:18px 0}
    .pc-list li{grid-template-columns:1fr;gap:0}
    .pc-list .n{margin-bottom:8px;display:block}
  }
...
<!-- ══════════════════════════════════════════════════════════════
     01 · REACH
     The address, above the fold, at the size of a headline. This is
     the whole page. Everything below it is there so somebody knows
     they are in the right place before they write.

     THE MAILTO CARRIES NO SUBJECT PARAMETER. A prefilled subject
     saves Daniel a second of triage and costs the sender the first
     line they were going to write, which is the line that says what
     they actually want. Leave it plain.
     ══════════════════════════════════════════════════════════════ -->
<section id="pc-reach">
  <div class="pc-wrap"><div class="grid"><div>

    <p class="pc-label pc-gold-label pc-up">Contact</p>
    <h1 class="pc-serif pc-up" style="animation-delay:.08s">Write to me.</h1>
    <p class="lede pc-up" style="animation-delay:.16s">It comes to my own inbox and I answer it myself. If you would rather message me where you already are, I read Instagram and LinkedIn too.</p>

    <div class="pc-rows pc-up" style="animation-delay:.26s">

      <a class="pc-row is-primary" href="mailto:daniel@parallaxxtransformations.com">
        <span>
          <span class="what">Email</span>
          <span class="who">daniel@<wbr>parallaxxtransformations.com</span>
        </span>
        <span class="go" aria-hidden="true">&#8594;</span>
      </a>

      <a class="pc-row" href="https://www.instagram.com/daniel.lawson__/" target="_blank" rel="noopener">
        <span>
          <span class="what">Instagram</span>
          <span class="who">&#64;daniel.lawson__</span>
        </span>
        <span class="go" aria-hidden="true">&#8599;</span>
      </a>
    ...
    <nav class="pc-social pc-up" style="animation-delay:.34s" aria-label="Elsewhere">
    ...
    <p class="pc-where pc-up" style="animation-delay:.42s">Clients across Europe, the US and Australia.</p>

    </div>

    <figure id="pc-portrait" class="pc-up" style="animation-delay:.3s">
      <img src="https://static.wixstatic.com/media/111174_5104ff2da2e0456a80264bc62d635fa4~mv2.jpg/v1/fill/w_1200,h_1500,al_c,q_90,enc_auto/daniel-conversation.jpg"
           alt="Daniel Lawson, mid conversation, looking at the camera"
  ...
  <div class="pc-wrap"><div class="pc-col">
    ...
    <ul class="pc-list">
      <li>
        ...
        <p><b>An event that needs facilitating</b>You are running a retreat or an offsite and you want people leaving different from how they arrived. Tell me who is coming, how long you have got, and what you want them walking out with.</p>
      </li>
      <li>
        <span class="n">02</span>
        <p><b>A stage or a podcast</b>Same thing, shorter. Who is listening, and what do you want them holding at the end of it.</p>
      </li>
      <li>
        <span class="n">03</span>
        <p><b>The group work, or the one to one</b>You have been reading and you want to know whether it is for you before you apply. Ask me. I would rather you asked than guessed.</p>
      </li>
      <li>
        <span class="n">04</span>
        <p><b>Something else</b>A partnership, a press question, or something that fits nowhere above. Send it anyway.</p>
      </li>
    </ul>
...
<!-- ══════════════════════════════════════════════════════════════
     03 · STILL LOOKING
     Contact sits in the nav on every page, so a share of the people
     landing here are still working out what this site is. Sending
     them back to the home page would be the same dead end the old
     301 was. Both doors carry a free instrument, so they get one.
     ══════════════════════════════════════════════════════════════ -->
<section id="pc-doors">
  <div class="pc-wrap"><div class="pc-col">

    <p class="pc-label pc-gold-label">Still looking</p>
    <h2>Working out where you fit?</h2>
    <p>Both doors carry something free, and you get the answer on the screen when you finish.</p>

    <div class="pc-doors-grid">

      <a class="pc-door" href="https://www.parallaxxtransformations.com/the-archetype-quiz">
        <span class="k">For Men</span>
        <span class="t">The Archetype Quiz</span>
        <span class="d">Five patterns. Find which one you are running, and what it is costing the people closest to you.</span>
      </a>

      <a class="pc-door" href="https://www.parallaxxtransformations.com/priority-audit">
        <span class="k">For Women</span>
        <span class="t">The Priority Audit</span>
        <span class="d">Fifteen statements about an ordinary week, and the three things they were measuring. About ninety seconds.</span>
      </a>
