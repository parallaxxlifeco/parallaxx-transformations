/* HAND-EDITED ON PURPOSE, 24 Aug 2026 -- and this is the one file in the repo
   where that is the correct move rather than a mistake.

   This bundle is AHEAD of its source. It carries a phone-column fix that
   PtFooter v3.dc.html never received, so running build-chrome-bundles.py
   silently reverts it -- see "Known defects carried over" in
   migration/REBUILD-QUEUE.md. Rebuilding this file to pick up a one-line link
   change would have traded one fix for another.

   So the same edit that went into PtFooter v3.dc.html and into all eleven
   pages that bake the footer in was applied here directly: the footer's
   "The Archetype Quiz" link now points at /the-archetype-quiz instead of
   /reconnect, which was the Reconnect programme and 301s to the home page.

   THE DRIFT IS STILL HERE. Port the phone-column fix back into
   PtFooter v3.dc.html and this file can go back to being generated.
*/
/* PARALLAXX PARALLAXX-FOOTER - GENERATED by build-chrome-bundles.py from "PtFooter v3.dc.html".
   DO NOT EDIT - edit the .dc.html and rerun the build.
   Tag: parallaxx-footer */
(function(){
  if (customElements.get('parallaxx-footer')) return;
...
/* ═══ PHONE COLUMNS · ported from the home page copy, 12 Aug ══════
   The home page carries its own built-in copy of this footer because
   PtFooter would not load in the Wix footer strip, and that copy got a
   phone fix this one never received. Held at three columns a 390px
   phone gives each 109px, and at .92rem "The Reconnected" cannot share
   a line with anything, so every column broke to one word per line and
   the block read as loose words rather than three lists.

   Two things fix it. Each column becomes a SUBGRID of the footer's own
   rows, so row 2 is the same height in all three whether its link
   wrapped once or three times. And the type is sized in cqw against
   the footer's own width rather than the viewport, because inside a
   Wix widget the viewport is not what we were given. 3.2cqw lands on
   11.2px, under the 11.75px that "The Reconnected" needs to hold a
   line.

   THIS BLOCK MUST STAY LAST. The existing max-width:440 rule sits
   ABOVE the base .pt-door a declaration, and a media query does not
   raise specificity, so its .78rem never won at any width. From down
   here these rules do.

   Browsers without subgrid ignore the declaration and get the previous
   layout, which is the right fallback. */
@media(max-width:560px){
  ...
  function boot(root, props){
    if(!root || !root.getElementById('pt-foot')) return;
  ...
  class ParallaxxFooter extends HTMLElement {
    connectedCallback(){
  ...
  customElements.define('parallaxx-footer', ParallaxxFooter);
})();
