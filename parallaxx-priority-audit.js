...
(function(){
  ...
  /* THE LIVE ROW IS THE ONE HORIZONTAL SCALE ON THE PAGE. Everywhere else
     .pa-scale is a column of five full-width targets, which is right when a
     statement owns the whole screen. Here five stacked options would make
     the intro taller than the phone and bury the promise under them, so
     this scale alone runs as a row. Scoped to .pa-live-row rather than
     changing .pa-scale, which the items screen depends on. */
  .pa-live-row .pa-scale{flex-direction:row;flex-wrap:nowrap;gap:8px;
  ...
  @media(max-width:400px){
    .pa-live-row .pa-scale{flex-wrap:wrap}
    .pa-live-row .pa-opt{flex:1 1 30%}
  }
