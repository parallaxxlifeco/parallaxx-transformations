/* PARALLAXX TRANSFORMATIONS - Speaking & Facilitating. Custom Element.
   ...
   THIS BUILD STILL CARRIES 4 EMPTY IMAGE SLOT(S): AFTER, HERO, ROOM, WORK. Run
   build-speaking-bundle.py --ship before deploying. */
(function(){
  if (customElements.get('parallaxx-speaking')) return;

  var CSS = `/* ═══════════════════════════════════════════════════════════════════
   PARALLAXX TRANSFORMATIONS  ·  SPEAKING & FACILITATING
   -------------------------------------------------------------------
   THIS IS A CREDIBILITY PAGE, NOT A SALES PAGE, and the difference is
   structural rather than cosmetic. Daniel's two live doors are the
   men's and women's avatars; an event organiser is a third buyer he
   is not currently chasing. So the page does not run a sales arc. It
   shows that he has stood in front of other people's audiences and
   been paid for it, which is what somebody checking him out came to
   find out, and it makes every other page on the site more believable
   on the way past.

   Practically that means: no urgency, no offer stack, no price, and
   one quiet way to write to him at the end rather than an ENQUIRE NOW
   in a gold pill. The old page ended in exactly that pill and it was
   the loudest thing on a page nobody had asked to be sold on.

   THE PROOF IS OTHER PEOPLE'S WORDS. Three organiser briefs, verbatim
   and unedited, carry more than any claim written here could. They
   are set as the largest type on the page after the headline, which
   is the whole argument of the layout.

   ONE PAGE, TWO FOOTER LINKS. /facilitating redirects here rather
   than getting a page of its own: it is the same offer described a
   second way, and splitting thin material across two URLs makes both
   of them worse.

   ═══════════════════════════════════════════════════════════════════
   THE IMAGE SLOTS
   -------------------------------------------------------------------
   Four photographs are marked IMAGE_SLOT_* in the source. The build
   renders each one as a labelled slot carrying its own brief, and
   REFUSES TO SHIP while any of them is still a slot -- run
   build-speaking-bundle.py with --ship to check. The old page pointed
   its hero at 'assets/stage-3.jpg', a file that has never existed in
   this repo, and shipped a broken image above the fold for months
   because nothing ever failed loudly about it. This is that lesson.
   ═══════════════════════════════════════════════════════════════════ */

  :where(#sp-root) *,:where(#sp-root) *::before,:where(#sp-root) *::after{box-sizing:border-box;margin:0;padding:0}
  #sp-root ::selection{background:#FF501F;color:#fff}
  ...
  .sp-serif{font-family:'Poppins','Montserrat',sans-serif;font-weight:400;letter-spacing:-.018em;line-height:1.14;color:var(--cream)}
  .sp-wrap{max-width:1240px;margin:0 auto;padding-left:clamp(20px,4vw,52px);padding-right:clamp(20px,4vw,52px)}
  .sp-col{max-width:680px;margin-left:auto;margin-right:auto}
  .sp-label{font-size:.66rem;letter-spacing:.22em;text-transform:uppercase;font-weight:700;color:var(--label)}
  .sp-gold-label{color:var(--gold);opacity:.9}
  #sp-root strong{font-weight:600;color:var(--cream)}
  .on-cream strong{color:var(--ink)}
  .sr{position:absolute;width:1px;height:1px;overflow:hidden;clip:rect(0 0 0 0);white-space:nowrap}

  .sp-up{animation:spUp .8s cubic-bezier(.22,.61,.36,1) both}
  ...
  /* ── IMAGE SLOTS ─────────────────────────────────────────────────
     A slot is a real element with a real aspect ratio, so the page
     lays out at its true height before a single photograph exists and
     nothing shifts when they land. */
  .sp-shot{position:relative;overflow:hidden;background:var(--navy-2)}
  .sp-shot img{display:block;width:100%;height:auto;object-fit:cover}
  .sp-slot{position:relative;display:flex;flex-direction:column;justify-content:flex-end;
    background:
      repeating-linear-gradient(135deg,rgba(232,198,95,.05) 0 12px,transparent 12px 24px),
      var(--navy-2);
    border:1px dashed rgba(232,198,95,.4);padding:clamp(20px,3vw,30px)}
  .sp-slot .tag{font-size:.6rem;letter-spacing:.24em;text-transform:uppercase;font-weight:700;
    color:var(--gold);margin-bottom:10px}
  .sp-slot .brief{font-family:'Poppins','Montserrat',sans-serif;font-size:clamp(.95rem,2vw,1.15rem);
    color:var(--cream);line-height:1.35;max-width:36ch;letter-spacing:-.01em}
  .sp-slot .dim{margin-top:10px;font-size:.76rem;color:var(--slate);letter-spacing:.06em}

  /* ── 01 · HERO ───────────────────────────────────────────────── */
  #sp-hero{position:relative;background:var(--navy-ink);min-height:min(88svh,780px);
    display:flex;align-items:flex-end;overflow:hidden}
  #sp-hero .shot{position:absolute;inset:0}
  #sp-hero .shot img{width:100%;height:100%;object-fit:cover;object-position:center 34%}
  /* THE HERO SLOT SITS TOP-LEFT, NOT CENTRED, because the headline occupies
     the middle of this section and a centred brief lands underneath it -- two
     pieces of type in the same place, which reads as a bug rather than as a
     slot waiting to be filled. It also carries z-index so the two veils, which
     exist to darken a photograph, do not darken the instructions instead. */
  /* z-index 1, not 4. It has to sit ABOVE the two veils, which are
     unpositioned and would otherwise darken the instructions, and BELOW
     .inner at z-index 3, which is the headline -- at 4 the slot's own
     gradient painted over the headline and washed it out. */
  #sp-hero .sp-slot{position:absolute;inset:0;z-index:1;
    justify-content:flex-start;align-items:flex-start;text-align:left;
    border:0;border-bottom:1px dashed rgba(232,198,95,.35);
    padding:calc(var(--sp-nav) + 20px) clamp(20px,4vw,52px) 0;
    background:
      repeating-linear-gradient(135deg,rgba(232,198,95,.05) 0 12px,transparent 12px 24px),
      linear-gradient(180deg,rgba(4,18,42,.72),rgba(3,12,28,.5))}
  #sp-hero .sp-slot .brief{max-width:46ch;font-size:clamp(.92rem,1.8vw,1.05rem)}
  #sp-hero .veil{position:absolute;inset:0;
    background:linear-gradient(180deg,rgba(3,12,28,.72) 0%,rgba(3,12,28,.24) 34%,rgba(4,18,42,.94) 100%)}
  #sp-hero .veil2{position:absolute;inset:0;
    background:linear-gradient(78deg,rgba(3,12,28,.76),transparent 62%)}
  #sp-hero .inner{position:relative;z-index:3;width:100%;
    padding:calc(clamp(80px,12vw,140px) + var(--sp-nav)) 0 clamp(50px,7vw,88px)}
  #sp-hero h1{font-size:clamp(2.3rem,7vw,4.4rem);font-weight:400;max-width:15ch;margin:16px 0 0}
  #sp-hero h1 em{font-style:normal;color:var(--gold)}
  #sp-hero .after{margin:26px 0 0;font-size:1.02rem;color:var(--mist);max-width:50ch;line-height:1.66}
  #sp-hero .after b{color:var(--cream);font-weight:600}

  /* ── 02 · WHAT IT IS ─────────────────────────────────────────── */
  #sp-what{background:var(--navy);padding:clamp(56px,8vw,100px) 0}
  #sp-what .big{font-family:'Poppins','Montserrat',sans-serif;font-weight:300;letter-spacing:-.02em;
    font-size:clamp(1.42rem,3.6vw,2.3rem);line-height:1.26;color:var(--cream);max-width:22ch}
  #sp-what .big em{font-style:normal;color:var(--gold)}
  #sp-what p+p{margin-top:18px}
  #sp-what .body{margin-top:clamp(26px,4vw,36px);color:var(--mist);max-width:58ch;font-size:1rem;line-height:1.7}

  /* ── 03 · WHY THEY BOOK HIM ──────────────────────────────────── */
  #sp-why{background:var(--cream);color:var(--ink-soft);padding:clamp(56px,8vw,100px) 0}
  #sp-why .sp-label{color:var(--bronze)}
  #sp-why h2{font-family:'Poppins','Montserrat',sans-serif;font-weight:400;letter-spacing:-.018em;
    color:var(--ink);font-size:clamp(1.5rem,3.4vw,2.1rem);line-height:1.2;margin:14px 0 0;max-width:22ch}
  .sp-briefs{margin:clamp(30px,4vw,46px) 0 0;display:grid;gap:14px;
    grid-template-columns:repeat(auto-fit,minmax(260px,1fr))}
  .sp-brief{background:var(--cream-2);border:1px solid var(--line-cream);border-radius:16px;
    padding:clamp(26px,3vw,34px)}
  .sp-brief blockquote{font-family:'Poppins','Montserrat',sans-serif;font-weight:300;letter-spacing:-.014em;
    font-size:clamp(1.08rem,2.3vw,1.4rem);line-height:1.32;color:var(--ink)}
  .sp-brief blockquote em{font-style:normal;color:var(--coral-ink);font-weight:400}
  .sp-brief .src{margin-top:18px;font-size:.66rem;letter-spacing:.2em;text-transform:uppercase;
    font-weight:700;color:var(--bronze)}
  #sp-why .after{margin:clamp(28px,4vw,38px) 0 0;color:var(--ink-soft);max-width:60ch;font-size:1rem;line-height:1.7}

  /* ── 04 · THE ROOM (photo band) ──────────────────────────────── */
  #sp-band{background:var(--navy-ink)}
  #sp-band .sp-shot,#sp-band .sp-slot{aspect-ratio:16/7;min-height:240px}
  #sp-band .sp-shot img{height:100%}
  #sp-band figcaption{max-width:1240px;margin:0 auto;padding:18px clamp(20px,4vw,52px) 0;
    font-size:.8rem;color:var(--slate);letter-spacing:.04em}

  /* ── 05 · PROOF ──────────────────────────────────────────────── */
  #sp-proof{background:var(--navy);padding:clamp(56px,8vw,100px) 0}
  .sp-quote{margin:clamp(24px,3vw,34px) 0 0;font-family:'Poppins','Montserrat',sans-serif;
    font-weight:300;letter-spacing:-.018em;font-size:clamp(1.24rem,3vw,1.9rem);line-height:1.32;
    color:var(--cream)}
  .sp-quote em{font-style:normal;color:var(--gold)}
  .sp-attrib{margin:clamp(24px,3vw,32px) 0 0;display:flex;align-items:center;gap:16px}
  .sp-attrib img{width:56px;height:56px;border-radius:50%;object-fit:cover;flex:none;
    border:1px solid rgba(232,198,95,.35)}
  .sp-attrib .n{font-size:.86rem;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:var(--gold)}
  .sp-attrib .r{font-size:.86rem;color:var(--slate);margin-top:3px}
  .sp-more{margin:clamp(26px,4vw,34px) 0 0;color:var(--mist);font-size:.98rem;max-width:58ch;line-height:1.7}
  .sp-logos{margin:clamp(38px,5vw,56px) 0 0;padding-top:clamp(28px,4vw,38px);
    border-top:1px solid var(--line);display:flex;flex-wrap:wrap;align-items:center;
    gap:clamp(22px,4vw,48px);opacity:.72}
  .sp-logos img{height:30px;width:auto;object-fit:contain;filter:grayscale(1) brightness(1.7)}
  .sp-logos-note{margin:18px 0 0;font-size:.72rem;letter-spacing:.14em;text-transform:uppercase;color:var(--label)}

  /* ── 06 · WHAT HE IS BROUGHT IN FOR ──────────────────────────── */
  #sp-scope{background:var(--cream);color:var(--ink-soft);padding:clamp(56px,8vw,96px) 0}
  #sp-scope .sp-label{color:var(--bronze)}
  #sp-scope h2{font-family:'Poppins','Montserrat',sans-serif;font-weight:400;letter-spacing:-.018em;
    color:var(--ink);font-size:clamp(1.42rem,3.2vw,2rem);line-height:1.22;margin:14px 0 0}
  .sp-three{margin:clamp(30px,4vw,44px) 0 0;display:grid;gap:0;
    grid-template-columns:repeat(auto-fit,minmax(240px,1fr));border-top:1px solid var(--line-cream)}
  .sp-three > div{padding:clamp(26px,3vw,34px) clamp(22px,2.4vw,30px);
    border-bottom:1px solid var(--line-cream);border-left:1px solid var(--line-cream)}
  .sp-three > div:first-child{border-left:0;padding-left:0}
  .sp-three .n{font-family:'Poppins','Montserrat',sans-serif;font-size:.8rem;font-weight:600;
    letter-spacing:.08em;color:var(--bronze)}
  .sp-three h3{margin:14px 0 10px;font-family:'Poppins','Montserrat',sans-serif;font-weight:500;
    letter-spacing:-.012em;font-size:1.14rem;color:var(--ink);line-height:1.26}
  .sp-three p{font-size:.94rem;line-height:1.6;color:var(--ink-soft)}

  .sp-split{margin:clamp(38px,5vw,58px) 0 0;display:grid;gap:clamp(28px,4vw,54px);
    grid-template-columns:repeat(auto-fit,minmax(280px,1fr))}
  .sp-split h4{font-size:.66rem;letter-spacing:.22em;text-transform:uppercase;font-weight:700;
    color:var(--bronze);margin-bottom:18px}
  .sp-rows{border-top:1px solid var(--line-cream)}
  .sp-rows div{padding:15px 0;border-bottom:1px solid var(--line-cream);
    font-family:'Poppins','Montserrat',sans-serif;font-size:clamp(1rem,2vw,1.16rem);color:var(--ink);
    letter-spacing:-.01em;line-height:1.3}
  .sp-rows div span{display:block;margin-top:5px;font-family:'Montserrat',sans-serif;font-size:.86rem;
    color:var(--ink-soft);letter-spacing:0;line-height:1.5}
  .sp-tags{display:flex;flex-wrap:wrap;gap:9px}
  .sp-tags span{font-size:.72rem;font-weight:600;letter-spacing:.1em;text-transform:uppercase;
    color:var(--ink-soft);border:1px solid var(--line-cream);border-radius:999px;padding:.68em 1.12em;
    background:var(--cream-2)}

  /* ── 07 · CLOSE ──────────────────────────────────────────────── */
  #sp-close{background:var(--navy-ink);padding:clamp(52px,7vw,90px) 0}
  #sp-close h2{font-family:'Poppins','Montserrat',sans-serif;font-weight:400;letter-spacing:-.018em;
    color:var(--cream);font-size:clamp(1.42rem,3.2vw,2rem);margin:14px 0 0;line-height:1.22;max-width:20ch}
  #sp-close p{margin:16px 0 0;color:var(--slate);font-size:.99rem;max-width:52ch;line-height:1.7}
  #sp-close p a{color:var(--mist);text-decoration:underline;text-underline-offset:3px;
    text-decoration-color:rgba(255,80,31,.6);font-weight:600;transition:color .25s}
  #sp-close p a:hover{color:var(--cream);text-decoration-color:var(--coral)}
  #sp-close p a:focus-visible{outline:2px solid var(--gold);outline-offset:3px}
  .sp-aside{margin:clamp(30px,4vw,42px) 0 0;padding-top:clamp(24px,3vw,32px);border-top:1px solid var(--line)}
  .sp-aside p{color:var(--label);font-size:.9rem;margin:0 0 14px}
  .sp-pills{display:flex;flex-wrap:wrap;gap:10px}
  .sp-pills a{font-size:.76rem;font-weight:600;letter-spacing:.1em;text-transform:uppercase;
  ...
  .sp-pills a:hover{border-color:var(--gold);color:var(--cream);background:rgba(232,198,95,.08)}
  .sp-pills a:focus-visible{outline:2px solid var(--gold);outline-offset:3px}

  /* ── PHONE ───────────────────────────────────────────────────── */
  @media(max-width:760px){
    .sp-three > div{border-left:0;padding-left:0;padding-right:0}
  }
  @media(max-width:560px){
    #sp-hero{min-height:auto}
    #sp-hero .inner{padding-top:calc(64px + var(--sp-nav));padding-bottom:44px}
    #sp-band .sp-shot,#sp-band .sp-slot{aspect-ratio:4/3}
    .sp-logos img{height:24px}
    .sp-attrib img{width:48px;height:48px}
  }

  /* ── THE TWO SPLIT SECTIONS ──────────────────────────────────────
     §02 and §07 both run text left, photograph right. Kept as one
     rule rather than two, because when the ratio changes it has to
     change in both places or the page develops a wobble down its
     right edge that nobody can name. */
  #sp-what .grid,#sp-close .grid{max-width:1080px;margin:0 auto;display:grid;
    grid-template-columns:minmax(0,1.1fr) minmax(0,.9fr);
    gap:clamp(34px,5vw,68px);align-items:center}
  #sp-work,#sp-work .sp-slot{border-radius:20px}
  #sp-work{border:1px solid var(--line)}
  #sp-work img{aspect-ratio:4/5}
  #sp-work .sp-slot{aspect-ratio:4/5;border:1px dashed rgba(232,198,95,.4)}
  #sp-after,#sp-after .sp-slot{border-radius:20px}
  #sp-after{border:1px solid var(--line)}
  #sp-after img{aspect-ratio:4/3}
  #sp-after .sp-slot{aspect-ratio:4/3;border:1px dashed rgba(232,198,95,.4)}

  @media(max-width:900px){
    #sp-what .grid,#sp-close .grid{grid-template-columns:1fr;gap:clamp(30px,6vw,42px)}
    #sp-work img,#sp-work .sp-slot{aspect-ratio:3/2}
    #sp-after img,#sp-after .sp-slot{aspect-ratio:3/2}
  }
...
<!-- ══════════════════════════════════════════════════════════════
     01 · HERO
     The headline is a client's brief, not a claim. "Strangers arrive,
     friends leave" is what a retreat host asked Daniel for, quoted on
     the old page in his own words, and on a page whose whole job is
     proof the customer's sentence outranks anything written here.
     ══════════════════════════════════════════════════════════════ -->
<section id="sp-hero">

  <figure class="shot sp-shot">
    <div class="sp-slot" role="img" aria-label="Photograph still to come: You mid-session with the audience in the frame. Wide, taken from the side or the back, and you off-centre -- the headline sits over the left third and the bottom third goes dark under the gradient."><p class="tag">Image slot &middot; HERO</p><p class="brief">You mid-session with the audience in the frame. Wide, taken from the side or the back, and you off-centre -- the headline sits over the left third and the bottom third goes dark under the gradient.</p><p class="dim">2400 x 1350 &middot; replace IMAGE_SLOT_HERO in the source</p></div>
  </figure>
  <div class="veil"></div>
  <div class="veil2"></div>

  <div class="inner">
    <div class="sp-wrap"><div class="sp-col">
      <p class="sp-label sp-gold-label sp-up">Speaking &amp; Facilitating</p>
      <h1 class="sp-serif sp-up" style="animation-delay:.1s">Strangers arrive. <em>Friends leave.</em></h1>
      <p class="after sp-up" style="animation-delay:.2s">That is what one organiser asked me for, and it is more or less what all of them are asking for. <b>Retreats, offsites and stages</b>, mostly multi-day, across Europe, the US and Australia.</p>
      <p class="after sp-up" style="animation-delay:.28s">This page is here so you can see it is real.</p>
    </div></div>
  </div>
...
<!-- ══════════════════════════════════════════════════════════════
     02 · WHAT IT IS
     The old page's own idea, kept: the intersection of future dreams
     and present experience. What changed is the sentence underneath
     it, which used to run "my aim is not to create better leaders,
     but rather to show individuals how to lead themselves" -- a
     reframe by negation, which is not how Daniel talks. It says what
     the work IS and lets the leaders line arrive as the aside it
     always was.
     ══════════════════════════════════════════════════════════════ -->
<section id="sp-what">
  <div class="sp-wrap"><div class="grid">

    <div>
      <p class="sp-label sp-gold-label">The work</p>
      <p class="big" style="margin-top:16px">I meet an audience where their <em>future dreams</em> and their <em>present experience</em> cross over.</p>
      <div class="body">
        <p>Most people working in personal leadership are trying to make better leaders. What I am doing is showing people how to lead themselves, toward a life that is healthier, more functional, and actually going somewhere they chose. Fulfilment and inner peace come out of that. Which, as it happens, makes better leaders too.</p>
        <p>The work is interactive and it moves. People get out of their heads and back into their hearts, and they do it in front of each other, which is the part that changes things.</p>
      </div>
    </div>

    <figure class="sp-shot" id="sp-work">
      <div class="sp-slot" role="img" aria-label="Photograph still to come: Closer, and portrait. You working with a handful of people rather than presenting to a full audience. Faces, hands, somebody mid-sentence."><p class="tag">Image slot &middot; WORK</p><p class="brief">Closer, and portrait. You working with a handful of people rather than presenting to a full audience. Faces, hands, somebody mid-sentence.</p><p class="dim">1200 x 1500 &middot; replace IMAGE_SLOT_WORK in the source</p></div>
    </figure>
...
<!-- ══════════════════════════════════════════════════════════════
     03 · WHY ORGANISERS BOOK HIM
     Three briefs, verbatim from the old page and unedited. They are
     set as the largest type on the page after the headline, which is
     the argument of the whole layout: his customers say it better.
     ══════════════════════════════════════════════════════════════ -->
<section id="sp-why" class="on-cream">
  <div class="sp-wrap"><div class="sp-col" style="max-width:1080px">

    <p class="sp-label">Why organisers book me</p>
    <h2>Three of them, in their own words.</h2>

    <div class="sp-briefs">
      <div class="sp-brief">
        <blockquote>&ldquo;To make his hardened sales team connect with their feelings and cry together.&rdquo;</blockquote>
        <p class="src">A sales organisation</p>
      </div>
      <div class="sp-brief">
        <blockquote>&ldquo;So that the attendees actually implement the technical elements of the training.&rdquo;</blockquote>
        <p class="src">A training retreat</p>
      </div>
      <div class="sp-brief">
        <blockquote>&ldquo;So my guests arrive as strangers and <em>leave as friends</em>.&rdquo;</blockquote>
        <p class="src">A retreat host</p>
      </div>
    </div>

    <p class="after">What they are hiring is interactive group work with a design built for their event rather than lifted off a shelf. And because I build it backwards from what they want people walking out with, organisers usually end up taking the structural and scheduling notes too. That is not what they came for, and it is the thing they mention afterwards.</p>
...
<!-- ══════════════════════════════════════════════════════════════
     04 · THE ROOM
     A full-width band between the claim and the proof. The caption is
     a [CLIENT] slot: name the event and the year, or delete the line.
     An uncaptioned photograph of an audience is decoration; a
     captioned one is evidence.
     ══════════════════════════════════════════════════════════════ -->
<section id="sp-band">
  <figure>
    <div class="sp-shot">
      <div class="sp-slot" role="img" aria-label="Photograph still to come: The audience, not you. Faces absorbed, phones down. Wide enough to read as a real event. This one carries a caption naming the event and the year, so pick a photograph you can name."><p class="tag">Image slot &middot; ROOM</p><p class="brief">The audience, not you. Faces absorbed, phones down. Wide enough to read as a real event. This one carries a caption naming the event and the year, so pick a photograph you can name.</p><p class="dim">2400 x 1050 &middot; replace IMAGE_SLOT_ROOM in the source</p></div>
    </div>
    <figcaption>[CLIENT] Name the event and the year here, or delete this line.</figcaption>
  </figure>
</section>
...
<!-- ══════════════════════════════════════════════════════════════
     05 · PROOF
     Joe Moose's words, verbatim from the live page. Nothing here is
     paraphrased and nothing is trimmed mid-sentence.
     [CLIENT] "Co-Founder, Solarcon" came off the old build, not off
     the harvested page, which named him only in an image filename.
     Confirm the title before this ships.
     ══════════════════════════════════════════════════════════════ -->
<section id="sp-proof">
  <div class="sp-wrap"><div class="sp-col">

    <p class="sp-label sp-gold-label">What an organiser said afterwards</p>

    <blockquote class="sp-quote">&ldquo;His facilitation style stood out, engaging participants and creating a welcoming, inclusive environment. Many attendees reported <em>significant improvements in their personal and professional lives</em> due to insights gained during Daniel&rsquo;s sessions.&rdquo;</blockquote>

    <div class="sp-attrib">
      <img src="https://static.wixstatic.com/media/e1784d_74e7b25af8bf453f8aca6e9c7341fee0~mv2.jpg/v1/crop/x_41,y_0,w_743,h_824/fill/w_160,h_178,al_c,q_85,enc_avif,quality_auto/329826931_1575276226277588_6345573999120963646_n5-ezgif_com-webp-to-jpg-converter.jpg"
           alt="Joe Moose" width="160" height="178" loading="lazy">
      <div>
        <p class="n">Joe Moose</p>
        <p class="r">Co-Founder, Solarcon &middot; Solar Villa retreat</p>
      </div>
    </div>

    <p class="sp-more">&ldquo;Hiring Daniel was a wise investment, with significant ROI. Participants continue to see long-term positive impacts. Daniel tailored his approach to meet our specific needs and adapted effortlessly to unexpected changes, ensuring sessions remained relevant and impactful.&rdquo;</p>

    <div class="sp-logos">
      <img src="https://static.wixstatic.com/media/e1784d_978cc98b14a14a548387e10c1a1accb0~mv2.png/v1/crop/x_86,y_0,w_8304,h_4418/fill/w_220,h_117,al_c,q_85,enc_avif,quality_auto/1-02.png" alt="Solarcon" loading="lazy">
      <img src="https://static.wixstatic.com/media/e1784d_0d18023878014cd193fca1e69ee17a39~mv2.webp/v1/fill/w_130,h_68,al_c,q_85,enc_avif,quality_auto/e1784d_0d18023878014cd193fca1e69ee17a39~mv2.webp" alt="ProTrader X" loading="lazy">
      <img src="https://static.wixstatic.com/media/e1784d_8540d4c3c9b94831a4e2a8533ed1d15b~mv2.png/v1/crop/x_0,y_463,w_1080,h_166/fill/w_300,h_46,al_c,q_85,enc_avif,quality_auto/contier%20(1).png" alt="Virtual Solar Club" loading="lazy">
      <img src="https://static.wixstatic.com/media/e1784d_2eeefa7c508d4983b0836638f833db50~mv2.png/v1/crop/x_0,y_358,w_1080,h_309/fill/w_210,h_60,al_c,q_85,enc_avif,quality_auto/contier.png" alt="Nomadic Closer" loading="lazy">
      <img src="https://static.wixstatic.com/media/e1784d_93e1dbdf647649508a130e959df3b7dc~mv2.png/v1/fill/w_142,h_126,al_c,q_85,enc_avif,quality_auto/e1784d_93e1dbdf647649508a130e959df3b7dc~mv2.png" alt="Cafe Coach Bali" loading="lazy">
    </div>
    <p class="sp-logos-note">Events and communities I have facilitated for</p>
...
<!-- ══════════════════════════════════════════════════════════════
     06 · SCOPE
     A reference block rather than a sell. Somebody who has already
     decided needs to know what is on the table and what kind of event
     this suits, and they need it in one screen without a form.
     ══════════════════════════════════════════════════════════════ -->
<section id="sp-scope" class="on-cream">
  <div class="sp-wrap"><div class="sp-col" style="max-width:1080px">

    <p class="sp-label">What I get brought in for</p>
    <h2>Three things, usually in this order.</h2>

    <div class="sp-three">
      <div>
        <p class="n">01</p>
        <h3>Event design</h3>
        <p>Planning the arc of the thing before it exists, so every session builds on the one before it instead of sitting beside it.</p>
      </div>
      <div>
        <p class="n">02</p>
        <h3>Group facilitation</h3>
        <p>The sessions themselves. Interactive, and built to get a group of people connecting with each other rather than watching someone at the front.</p>
      </div>
      <div>
        <p class="n">03</p>
        <h3>One to one, for your attendees</h3>
        <p>Private coaching alongside the event, for the people who want to go further than a group session lets them.</p>
      </div>
    </div>

    <div class="sp-split">
      <div>
        <h4>Where this works best</h4>
        <div class="sp-rows">
          <div>Multi-day and multi-week immersions<span>Full immersion, small enough that people are known by the end of it.</span></div>
          <div>Lifestyle creation retreats<span>Where the outcome is how somebody lives afterwards.</span></div>
          <div>Strategy and education retreats<span>The how-to, with the self-management that makes it stick.</span></div>
          <div>Breathwork and meditation retreats<span>For the integration, so it comes home with them.</span></div>
          <div>Community and relationship retreats<span>Connection and belonging, within themselves and with each other.</span></div>
        </div>
      </div>
      <div>
        <h4>Industries I have worked in</h4>
        <div class="sp-tags">
          <span>Remote Sales</span>
...
<!-- ══════════════════════════════════════════════════════════════
     07 · CLOSE
     A credibility page ends in an address, not in a pill that says
     ENQUIRE NOW. The old page ended in exactly that, in gold, and it
     was the loudest thing on a page nobody had asked to be sold on.

     The aside underneath exists because this URL is linked from the
     footer of every page on the site, so a share of the people
     reading it arrived for themselves rather than for an event.
     ══════════════════════════════════════════════════════════════ -->
<section id="sp-close">
  <div class="sp-wrap"><div class="grid">

    <div>
      <p class="sp-label sp-gold-label">If you are putting something together</p>
      <h2>Write to me about it.</h2>
      <p>Tell me who is coming, how long you have got, and what you want them walking out with. <a href="mailto:daniel@parallaxxtransformations.com">daniel@parallaxxtransformations.com</a> &mdash; it comes to my own inbox.</p>

      <div class="sp-aside">
        <p>Here for yourself rather than for an event?</p>
        <nav class="sp-pills" aria-label="The two doors">
          <a href="https://www.parallaxxtransformations.com/men">For Men</a>
          <a href="https://www.parallaxxtransformations.com/women">For Women</a>
          <a href="https://www.parallaxxtransformations.com/about-daniel-lawson">About Daniel</a>
        </nav>
      </div>
    </div>

    <figure class="sp-shot" id="sp-after">
      <div class="sp-slot" role="img" aria-label="Photograph still to come: The end of it. People talking to each other rather than to you -- this is the photograph that proves the headline, so it matters more than it looks like it does."><p class="tag">Image slot &middot; AFTER</p><p class="brief">The end of it. People talking to each other rather than to you -- this is the photograph that proves the headline, so it matters more than it looks like it does.</p><p class="dim">1600 x 1200 &middot; replace IMAGE_SLOT_AFTER in the source</p></div>
    </figure>
...
<footer id="pt-foot">
  ...
  function addFonts(){
    ...
    var ff=document.createElement('style'); ff.id='sp-fonts'; ff.textContent=`/* LUMIOS MARKER is not on Google Fonts -- upload the .woff2 to Wix Media and
...
@font-face{font-family:'Lumios Marker';font-display:swap;
  ...
  function boot(root){
    if(!root || !root.getElementById('sp-root')) return;
...
/* ================================================================
  ...
  customElements.define('parallaxx-speaking', ParallaxxSpeaking);
})();
