...
(function(){
  ...
  var CSS = `/* ═══════════════════════════════════════════════════════════════════
   ...
   ═══════════════════════════════════════════════════════════════════
   TWO PHOTOGRAPHS, NOT FOUR -- AND WHY THAT CHANGED
   -------------------------------------------------------------------
   This page went live carrying four dashed placeholder boxes, each
   printing its own brief, addressed to Daniel in the second person:
   'You mid-session with the audience in the frame.' Visible on the
   public site.

   The slot system was built to prevent exactly that, and it failed for
   a reason worth writing down. The guard lived in
   build-speaking-bundle.py --ship, which is opt-in, and which nobody
   ran. THE THING THAT ACTUALLY DEPLOYS IS migration/build-site.py, and
   it never looked. A guard that is not in the deploy path is not a
   guard. build-site.py refuses to build a route whose bundle still
   contains a slot now, so no page can repeat this.

   The page is built around the two real photographs the archive holds:
   Daniel mid-session with the audience, and the audience on its own.
   The other two sections lost their images rather than keep a brief
   sitting where a photograph should be.

   THE SLOT MECHANISM, STILL HERE
   -------------------------------------------------------------------
   Any remaining slot is written IMAGE_SLOT_<NAME> in the source. The build
  ...
  /* ── THE TWO SPLIT SECTIONS ──────────────────────────────────────
     Both ran text left, photograph right, until the photographs that
     were supposed to fill them turned out not to exist. A two-column
     grid with one column empty is not a layout, it is a hole -- so
     both are a single measured column now. To widen them back out
     when there is real material, restore the two-column grid here and
     put the figure back in the markup. */
  #sp-what .grid,#sp-close .grid{max-width:720px;margin:0 auto}
...
<section id="sp-hero">
  ...
  <figure class="shot sp-shot">
    <img src="https://static.wixstatic.com/media/111174_107f479d04e9492189d6c7928e1ae8e5~mv2.jpg" alt="Daniel Lawson facilitating, the audience in frame" width="2400" height="1350">
  </figure>
  <div class="veil"></div>
  <div class="veil2"></div>
...
<!-- ══════════════════════════════════════════════════════════════
     04 · THE ROOM
     A full-width band between the claim and the proof. It carried a
     caption reading '[CLIENT] Name the event and the year here' -- a
     note to Daniel, live on the public site, exactly like the four
     image slots above it. Removed rather than guessed at: nobody in
     this repo knows which event this is.

     A captioned photograph is evidence and an uncaptioned one is
     decoration, so this is worth coming back to. Add the caption when
     the event and the year are confirmed.
     ══════════════════════════════════════════════════════════════ -->
<section id="sp-band">
  <figure>
    <div class="sp-shot">
      <img src="https://static.wixstatic.com/media/e1784d_c305d407e8e046b6b9f20129ebf7ee40~mv2.jpg/v1/fill/w_147,h_67,al_c,q_80,usm_0.66_1.00_0.01,blur_2,enc_avif,quality_auto/e1784d_c305d407e8e046b6b9f20129ebf7ee40~mv2.jpg" alt="An audience during one of Daniel Lawson's sessions" width="2400" height="1050">
    </div>
  </figure>
</section>
